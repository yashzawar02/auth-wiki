...
In the OIDC authentication process, a client application (relying party) initiates authentication by redirecting the user to an authorization server. After successful authentication, the server responds with an ID token, among other tokens like the access token. The ID token is typically encoded as a JWT (JSON Web Token) and includes key claims such as the user’s identifier (sub), the issuer (iss), audience (aud) and other user specific information.
