...
OAuth 2.1 é uma atualização proposta para o framework de autorização <Ref slug="oauth-2.0" />. Envolve uma série de mudanças e recomendações para a especificação existente do OAuth 2.0, que consolida as melhores práticas e melhorias de segurança amplamente adotadas na indústria ao longo dos anos.
...
2. Exigir o uso de <Ref slug="pkce" /> para todos os clientes, incluindo <Ref slug="client" headingId="confidential-clients">clientes confidenciais (privados)</Ref>.
...
O ROPC grant permite que o cliente troque as credenciais do usuário diretamente por um access token. Foi projetado para aplicações legadas que não podem suportar o authorization code flow. No entanto, o grant apresenta riscos de segurança por:
...
## Exigência de PKCE para todos os clientes
...
sequenceDiagram
    ...
    Client->>Client: Gerar verificador de código e desafio
    Client->>Authorization Server: Solicitar authorization code com desafio
    Authorization Server->>Authorization Server: Completar fluxo de autorização
    Authorization Server->>Client: Emitir authorization code
    Client->>Authorization Server: Trocar código por token com verificador
    Authorization Server->>Client: Emitir access token
```

Inicialmente, era recomendado que <Ref slug="client" headingId="public-clients">clientes públicos</Ref> usassem PKCE, mas o OAuth 2.1 estende essa recomendação para um requisito obrigatório para todos os clientes, incluindo <Ref slug="client" headingId="confidential-clients">clientes confidenciais (privados)</Ref>.
...
OAuth 2.0 não define explicitamente os tipos de cliente. Você pode ver várias categorizações na indústria, como por nível de acesso (público vs. confidencial) ou por tipo de aplicação (aplicativo web vs. aplicativo móvel). Para o framework OAuth, não importa como o cliente é implementado (já que eles são mais sobre os atributos de negócios do cliente), mas o nível de acesso faz diferença nos requisitos de segurança.

Assim, o OAuth 2.1 introduz uma definição clara dos tipos de cliente:
...
- <Ref slug="client" headingId="confidential-clients" />: Clientes que PODEM manter a confidencialidade de suas credenciais (por exemplo, aplicativos web do lado do servidor, aplicativos de desktop nativos).

## Requisitos de segurança para refresh tokens

<Ref slug="refresh-token">Refresh tokens</Ref> são tokens de longa duração usados pelo cliente para obter novos access tokens sem interação do usuário. Ao mesmo tempo, eles também são alvos de alto valor para atacantes. Como clientes públicos não podem armazenar credenciais de forma segura, o OAuth 2.1 especifica que o <Ref slug="authorization-server" /> deve usar um dos seguintes métodos para proteger refresh tokens:

- Emitir <Ref slug="refresh-token" headingId="sender-constrained-refresh-tokens">refresh tokens restritos ao remetente</Ref>.
- Usar <Ref slug="refresh-token" headingId="refresh-token-rotation">rotação de refresh token</Ref> para limitar a usabilidade e a vida útil dos refresh tokens.
