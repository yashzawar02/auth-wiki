...
description: WebAuthn é uma API para acessar credenciais de chave pública, facilitando a implementação de passkeys.
---

## O que é WebAuthn?

WebAuthn (Web Authentication API), desenvolvido pelo [W3C](https://www.w3.org/) e [FIDO Alliance](https://fidoalliance.org/), é uma especificação para autenticação web segura usando os padrões FIDO2. WebAuthn fornece uma API para permitir que sites implementem <Ref slug="passkey" />, que são credenciais resistentes a phishing, protegidas por criptografia de chave pública. Passkeys podem substituir senhas tanto para logins sem senha quanto para <Ref slug="mfa" />.
...
1. **Usuário:** O indivíduo que tenta acessar um aplicativo web.
2. **Agente do usuário:** O navegador web que lida com as chamadas da API WebAuthn e gerencia o processo de autenticação entre o usuário, a parte confiável e o autenticador.
3. **Parte confiável:** O serviço, aplicativo ou servidor de API que o usuário busca acessar.
4. **Autenticador:** O componente de hardware ou software usado para verificar a identidade do usuário. Pode assumir várias formas com base na capacidade da plataforma ou navegador, como chaves de segurança (como Yubikeys), telefones ou tablets (conectados por Bluetooth, NFC ou USB), biometria baseada em dispositivos ou PINs, etc.
...
Quando o usuário tenta registrar uma passkey, a parte confiável envia um desafio de registro para o agente do usuário.
3. **Verificação do usuário:**
O agente do usuário encaminha o desafio para o autenticador, que solicita a verificação do usuário (por exemplo, autenticação biométrica ou uma chave de segurança de hardware).
...
O agente do usuário envia o desafio assinado de volta para a parte confiável, que verifica a assinatura usando a chave pública e completa o processo de registro.

```mermaid
sequenceDiagram
	participant Usuário
	participant Agente do Usuário (Cliente)
	participant Parte Confiável (Servidor de API) 
	participant Autenticador

	Usuário ->> Agente do Usuário (Cliente): Solicitar registro de uma nova passkey
	Agente do Usuário (Cliente) ->> Parte Confiável (Servidor de API): Solicitar registro
	Parte Confiável (Servidor de API) -->> Agente do Usuário (Cliente): Enviar desafio e opções de registro
	Agente do Usuário (Cliente) ->> Autenticador: Solicitar interação do usuário
	Autenticador -->> Agente do Usuário (Cliente): Assinar desafio e criar credencial
	Agente do Usuário (Cliente) ->> Parte Confiável (Servidor de API): Enviar desafio assinado e credencial
	Parte Confiável (Servidor de API) -->> Agente do Usuário (Cliente): Verificar e armazenar credencial
	Agente do Usuário (Cliente) ->> Usuário: Registro bem-sucedido
```

### Autenticação WebAuthn

1. **Desafio de autenticação:**
Quando o usuário tenta fazer login, a parte confiável envia um desafio de autenticação para o agente do usuário.
2. **Verificação do usuário:**
O agente do usuário envia o desafio para o autenticador, que solicita a verificação do usuário (por exemplo, autenticação biométrica ou uma chave de segurança de hardware).
3. **Assinatura criptográfica:**
O autenticador usa sua chave privada para assinar o desafio, criando uma assinatura criptográfica.
4. **Verificação e acesso:**
O agente do usuário verifica a assinatura usando a chave pública e informa a parte confiável sobre uma autenticação bem-sucedida. O acesso é concedido se a verificação for bem-sucedida.
...
sequenceDiagram
	...
	Usuário ->> Agente do Usuário (Cliente): Iniciar login
	Agente do Usuário (Cliente) ->> Parte Confiável (Servidor de API): Solicitar autenticação
	Parte Confiável (Servidor de API) -->> Agente do Usuário (Cliente): Enviar desafio
	Agente do Usuário (Cliente) ->> Autenticador: Solicitar interação do usuário
	Autenticador -->> Agente do Usuário (Cliente): Assinar desafio
	Agente do Usuário (Cliente) ->> Parte Confiável (Servidor de API): Enviar desafio assinado
	Parte Confiável (Servidor de API) -->> Agente do Usuário (Cliente): Verificar assinatura e autenticar usuário
	Agente do Usuário (Cliente) ->> Usuário: Autenticação bem-sucedida
```
...
- **Fase de registro**: Os usuários completam um processo de registro padrão do <Ref slug="openid-connect" /> (por exemplo, verificação de e-mail e criação de senha). O WebAuthn gera um par de chaves pública-privada, armazenando a chave pública com o <Ref slug="identity-provider" /> do OIDC e mantendo a chave privada no dispositivo do usuário.
