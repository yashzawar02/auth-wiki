...
tags: [oauth 2.0, oidc]
description: Um token de acesso (access token) é uma credencial usada para acessar recursos protegidos em nome de uma identidade (por exemplo, usuário ou serviço). É um token de portador que concede acesso a recursos com base nos scopes (permissões) do token.
---
...
- <Ref slug="jwt" />: Um token autônomo que contém <Ref slug="claim">claims</Ref> (por exemplo, ID do usuário, tempo de expiração) com uma assinatura digital. O servidor de recursos pode validar o token sem fazer uma solicitação adicional ao servidor de autorização.
...
sequenceDiagram
    ...
    Servidor de Recursos->>Servidor de Autorização: Solicitar Conjunto de Chaves JSON Web (JWKS)
    ...
    Servidor de Recursos->>Servidor de Recursos: Validar assinatura e claims do JWT
    Servidor de Recursos->>Cliente: Enviar resposta
```
...
| Extensibilidade| Limitada                       | Claims personalizados                                         |
| Stateless      | Não                            | Sim                                                           |
| Segurança      | Requer validação do token      | Requer validação de assinatura                                |
| Padrão         | Não                            | Sim (RFC 7519)                                                |

Para mais informações sobre a escolha entre os dois tipos de tokens, veja [Token opaco vs JWT](https://blog.logto.io/opaque-token-vs-jwt).
...
- Emite tokens de acesso para clientes após autenticação e autorização bem-sucedidas. O servidor de autorização pode reduzir os scopes (reduzir os scopes para um subconjunto) ou rejeitar a solicitação de token com base nas políticas de controle de acesso (por exemplo, consentimento do usuário, <Ref slug="rbac" />, <Ref slug="abac" />).
...
- Fornece informações sobre o token (por exemplo, scopes, tempo de expiração) via introspecção de token ou <Ref slug="userinfo-endpoint" />.

Você pode notar que o servidor de autorização não interpreta o significado do token de acesso. Por exemplo, o token de acesso pode conter um scope `read:orders`, mas o servidor de autorização não sabe o que o scope significa. O servidor de recursos é responsável por interpretar o token de acesso e aplicar o <Ref slug="access-control" /> com base nos scopes do token. Ou seja, o <Ref slug="resource-server" /> geralmente tem as seguintes responsabilidades:

- Valida os <Ref slug="claim">claims</Ref> no token de acesso (por exemplo, tempo de expiração, indicador de recurso, scopes).
