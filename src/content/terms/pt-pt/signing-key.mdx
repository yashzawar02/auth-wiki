...
tags: [oidc]
description: Uma chave de assinatura é uma chave criptográfica usada para assinar e verificar JSON Web Tokens em OpenID Connect (OIDC). É usada para garantir a integridade e autenticidade dos tokens emitidos pelo fornecedor OpenID.
---

## O que é uma chave de assinatura?

No contexto de <Ref slug="openid-connect" />, uma **chave de assinatura**, geralmente um par de chaves assimétricas, é usada para assinar e verificar <Ref slug="jwt">JSON Web Tokens (JWTs)</Ref>. Os fornecedores OpenID usam chaves de assinatura para assinar tokens como <Ref slug="id-token">ID tokens</Ref> e <Ref slug="access-token">access tokens</Ref> para garantir sua integridade e autenticidade.
...
Quando um utilizador se autentica com um fornecedor OpenID, o fornecedor emite um ID token que contém informações do utilizador (<Ref slug="claim">claims</Ref>) e é assinado pela chave de assinatura do fornecedor. Como o ID token é um JWT, ele consiste em três partes: cabeçalho, payload e assinatura.
...
O ID token está agora pronto para ser enviado ao <Ref slug="client" /> para processamento posterior.

### 5. Verificar o token

Quando o cliente recebe o ID token, ele pode verificar a assinatura usando a chave pública do fornecedor OpenID. Normalmente, a chave pública é disponibilizada através do endpoint <Ref slug="openid-connect-discovery" /> (`jwks_uri`) no formato de um <Ref slug="jwk" headingId="json-web-key-set-jwks" />.
